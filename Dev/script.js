...
function initGSAP() {
  ...
  // ── Reveal animations ──
  // CSS handles initial hidden state (.reveal { opacity:0 })
  // We add .visible class to trigger CSS transitions, same as IO fallback
  gsap.utils.toArray('.reveal').forEach(el => {
    // Hero elements reveal immediately
    if (el.closest('#hero')) {
      const delay = el.classList.contains('stagger-1') ? 80 :
                    el.classList.contains('stagger-2') ? 160 :
                    el.classList.contains('stagger-3') ? 240 :
                    el.classList.contains('stagger-4') ? 320 : 0;
      setTimeout(() => el.classList.add('visible'), 150 + delay);
      return;
    }
  ...
  // ── Hero count-up numbers fire immediately ──
  setTimeout(() => {
    document.querySelectorAll('#hero .count-num').forEach(el => countUp(el));
  }, 300);
}
