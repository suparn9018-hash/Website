...
/* ══════════════════════════════════════════════════════════
   FRAMER-STYLE ANIMATION PASS
   1. Lenis smooth-scroll (momentum)
   2. Word-by-word reveal on section headlines
   3. Cursor-following blue glow (desktop only)
   4. Hero background parallax (cosmic image at 0.4x scroll)
══════════════════════════════════════════════════════════ */
(function () {
  ...
  if (!reducedMotion && typeof Lenis !== 'undefined') {
    try {
      lenisInstance = new Lenis({
        ...
        smoothWheel: true,
        smoothTouch: false,         /* native scroll on phones — feels better */
        wheelMultiplier: 1.0,
        touchMultiplier: 1.2,
      });
      ...
      /* Hijack in-page anchor links so Lenis handles the scroll */
  ...
  function splitWords(el) {
    ...
    el.dataset.split = '1';
    /* Walk text nodes and wrap each whitespace-delimited word in a clip-mask span */
    var textNodes = [];
  ...
  /* Apply word-reveal to ALL main headings on the page (h1 and h2.reveal)
     EXCEPT ones containing .grad-shimmer (splitting words inside the
     shimmer span would reset the gradient per word and look broken). */
  if (!reducedMotion) {
    var headlines = document.querySelectorAll(
      'h1.reveal, h2.reveal, .bf-h, .wyd-head h2, .why-intro h2, .gpus-header h2'
    );

    headlines.forEach(function (el) {
      if (el.dataset.split === '1') return;       /* already processed */
      if (el.querySelector('.grad-shimmer')) return;  /* skip hero shimmer */
      splitWords(el);
